...
class BboxCrop(object):
    """Crop the given Image by single bbox.
    ...
    def __init__(self,
                 #crop_size,
                 bbox=[0.0, 0.0, 479.0, 847.0],
                 interpolation='bilinear',
                 backend='cv2'):

        #assert len(np.array(bbox)) == 4
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            # 20220630 by gzb. for inference, need add bbox. 
            #print ('bbox******: {}'.format(bbox))
            if bbox is None:
                bbox = self.bbox
            #print ('bbox******: {}'.format(bbox))
            #bbox = [290.0, 121.0, 530.0, 361.0]

            img = results[key]
            img = mmcv.imcrop(img, bboxes=np.array(bbox))
    ...
    def __repr__(self):
        repr_str = self.__class__.__name__ + f'(bbox=used in __call__'
...
class ImageRescale(object):
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            img = results[key]
            img = mmcv.imrescale(img, float(scale), backend=self.backend)
            img_shape = img.shape

            results[key] = img

        results['img_shape'] = img_shape
        #print ('img shape is {}'.format(img_shape))
        #if img_shape[0] >= 283:
        #    print ('img shape is {}'.format(img_shape))
        #    print ('img name is {}'.format(results['img_info']))
        #if img_shape[1] >= 278:
        #    print ('img shape is {}'.format(img_shape))
        #    print ('img name is {}'.format(results['img_info']))
    ...
    def __repr__(self):
        repr_str = self.__class__.__name__ + f'(scale=used in __call__'
        #repr_str += f', efficientnet_style={self.efficientnet_style}'
        #repr_str += f', crop_padding={self.crop_padding}'
        repr_str += f', interpolation={self.interpolation}'
        repr_str += f', backend={self.backend})'
