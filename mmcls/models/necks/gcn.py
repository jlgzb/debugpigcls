...
class unit_adjacent(nn.Module):
    def __init__(self):
        ...
        self.conv_query = nn.Conv2d(in_channels=2560, out_channels=2560, kernel_size=1)
        self.conv_key = nn.Conv2d(in_channels=2560, out_channels=2560, kernel_size=1)
...
class unit_gcn(nn.Module):
    def __init__(self, in_C=2560, out_C=512):
...
class UnitGCNGMP(nn.Module):
    ...
    """
    def __init__(self, dim=2):
        super(UnitGCNGMP, self).__init__()
        assert dim in [1, 2, 3], 'GlobalAveragePooling dim only support ' \
            f'{1, 2, 3}, get {dim} instead.'
...
class UnitGCNTwo(nn.Module):
    ...
    """
    def __init__(self, dim=2):
        super(UnitGCNTwo, self).__init__()
        assert dim in [1, 2, 3], 'GlobalAveragePooling dim only support ' \
            f'{1, 2, 3}, get {dim} instead.'

        self.adjacent = unit_adjacent()
        self.gcn1 = unit_gcn(in_C=2560, out_C=2560)
        self.gcn2 = unit_gcn(in_C=2560, out_C=2560)
...
class UnitGCNThree(nn.Module):
    ...
    """
    def __init__(self, dim=2):
        super(UnitGCNThree, self).__init__()
        assert dim in [1, 2, 3], 'GlobalAveragePooling dim only support ' \
            f'{1, 2, 3}, get {dim} instead.'
        ...
        self.gcn1 = unit_gcn(in_C=2560, out_C=2560)
        self.gcn2 = unit_gcn(in_C=2560, out_C=2560)
        self.gcn3 = unit_gcn(in_C=2560, out_C=2560)
